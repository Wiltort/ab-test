    ...
    def test_device(self):
        ...
        for i in range(10):
            r = self.client.get(path='/api/experiments/',
                                headers={'Device-Token': self.tok})
            self.assertEqual(
                res.content, r.content, msg='Данные опыта изменились'
            )
    ...
    def test_distribution(self):
        ...
        for i in range(1000):
            ...
            for item in res.data:
                if item["key"] == "button_color":
                    experiment_1[item["value"]] = (
                        experiment_1.get(item["value"], 0) + 1
                    )
                elif item["key"] == "price":
                    experiment_2[item["value"]] = (
                        experiment_2.get(item["value"], 0) + 1
                    )
        ...
        for key, value in experiment_2.items():
            ...
            option, created = Option.objects.get_or_create(
                experiment=experiment, value=key
            )
            self.assertFalse(created)
            self.assertLessEqual(
